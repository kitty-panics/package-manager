...
:Execute_Uninstall
    ...
    taskkill /F /T /IM "chrome_proxy.exe"
    ...
    :: Delete shortcut
    del /F /Q "%HOMEDRIVE%\MyProgram\Shortcut\gc.lnk"
GOTO:EOF
...
:Execute_Install
    ...
    echo set oShellLink = SC.CreateShortcut("%HOMEDRIVE%\MyProgram\Shortcut\gc.lnk") >> "Shortcut.vbs"
    echo oShellLink.TargetPath = "%HOMEDRIVE%\MyProgram\%FileName%\chrome.exe"       >> "Shortcut.vbs"
    echo oShellLink.IconLocation = "%HOMEDRIVE%\MyProgram\%FileName%\chrome.exe,0"   >> "Shortcut.vbs"
