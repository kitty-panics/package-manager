...
:Execute_Uninstall
    ...
    taskkill /F /T /IM "cmake-gui.exe"
    taskkill /F /T /IM "cmake.exe"
    taskkill /F /T /IM "cmcldeps.exe"
    taskkill /F /T /IM "cpack.exe"
    taskkill /F /T /IM "ctest.exe"
    ...
    :: Delete shortcut
    del /F /Q "%HOMEDRIVE%\MyProgram\Shortcut\cm.lnk"
GOTO:EOF
...
:Execute_Install
    ...
    echo set oShellLink = SC.CreateShortcut("%HOMEDRIVE%\MyProgram\Shortcut\cm.lnk")      >> "Shortcut.vbs"
    echo oShellLink.TargetPath = "%HOMEDRIVE%\MyProgram\%FileName%\bin\cmake-gui.exe"     >> "Shortcut.vbs"
    echo oShellLink.IconLocation = "%HOMEDRIVE%\MyProgram\%FileName%\bin\cmake-gui.exe,0" >> "Shortcut.vbs"
